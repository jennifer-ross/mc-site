...
<div
	x-data="{ sidebarClose: false, currentUrl: window.location.pathname, chatPattern: /^\/chat(s|\/[\w-]+)$/, $activeUrlClass: '[&]:bg-lightblue-50 [&]:text-primary' }">
	<div
	...
	>
		<div class="text-white text-center justify-between flex gap-2">
			...
			<x-ui.button @click="sidebarClose = !sidebarClose" class="bg-primary-100">
				<template x-if="sidebarClose">
					<x-icon name="bx-menu" class="{{ $iconClass }}" />
				</template>
				<template x-if="!sidebarClose">
					<x-icon name="bx-menu-alt-right" class="{{ $iconClass }}" />
				</template>
			</x-ui.button>
		</div>
		<div class="py-6">
		...
		<nav class="flex flex-1 w-full mb-6">
			<ul class="flex flex-col gap-2 w-full h-full">
				<x-ui.menu.item x-bind:class="currentUrl === '/' ? $activeUrlClass : ''">
					...
					>
						<x-icon name="bx-grid-alt" class="{{ $iconClass }} {{ $iconMargin }}"
								x-bind:class="sidebarClose ? '[&]:mr-0' : ''" />
				...
				<x-ui.menu.item x-bind:class="currentUrl === '/dashboard' ? $activeUrlClass : ''">
					<a class="p-3 inline-flex items-center justify-start w-full" href="#"
					   x-bind:class="sidebarClose ? 'justify-center' : ''">
						<x-icon name="bx-user" class="{{ $iconClass }} {{ $iconMargin }}"
								x-bind:class="sidebarClose ? '[&]:mr-0' : ''" />
				...
				@can('view_any_chat')
					<x-ui.menu.item x-bind:class="chatPattern.test(currentUrl) ? $activeUrlClass : ''">
						<a
							wire:navigate
							href="/chats"
							class="p-3 inline-flex items-center justify-start w-full"
							x-bind:class="sidebarClose ? 'justify-center' : ''"
						>
							<x-icon name="bx-chat" class="{{ $iconClass }} {{ $iconMargin }}"
									x-bind:class="sidebarClose ? '[&]:mr-0' : ''" />
				...
				<x-ui.menu.item x-bind:class="currentUrl === '/courts' ? $activeUrlClass : ''">
					<a class="p-3 inline-flex items-center justify-start w-full" href="#"
					   x-bind:class="sidebarClose ? 'justify-center' : ''">
						<x-icon name="bx-pie-chart-alt-2" class="{{ $iconClass }} {{ $iconMargin }}"
								x-bind:class="sidebarClose ? '[&]:mr-0' : ''" />
				...
				<x-ui.menu.item x-bind:class="currentUrl === '/cards' ? $activeUrlClass : ''">
					<a class="p-3 inline-flex items-center justify-start w-full" href="#"
					   x-bind:class="sidebarClose ? 'justify-center' : ''">
						<x-icon name="bx-credit-card" class="{{ $iconClass }} {{ $iconMargin }}"
								x-bind:class="sidebarClose ? '[&]:mr-0' : ''" />
				...
				<x-ui.menu.item x-bind:class="currentUrl === '/subscribe' ? $activeUrlClass : ''">
					<a class="p-3 inline-flex items-center justify-start w-full" href="#"
					   x-bind:class="sidebarClose ? 'justify-center' : ''">
						<x-icon name="bx-cart-alt" class="{{ $iconClass }} {{ $iconMargin }}"
								x-bind:class="sidebarClose ? '[&]:mr-0' : ''" />
				...
				@can('view_any_user')
					<x-ui.menu.item x-bind:class="currentUrl === '/players' ? $activeUrlClass : ''">
						<a class="p-3 inline-flex items-center justify-start w-full" href="#"
						   x-bind:class="sidebarClose ? 'justify-center' : ''">
							<x-icon name="bx-heart" class="{{ $iconClass }} {{ $iconMargin }}"
									x-bind:class="sidebarClose ? '[&]:mr-0' : ''" />
				...
				@can('update_user', $user)
					<x-ui.menu.item x-bind:class="currentUrl === '/settings' ? $activeUrlClass : ''">
						<a class="p-3 inline-flex items-center justify-start w-full" href="#"
						   x-bind:class="sidebarClose ? 'justify-center' : ''">
							<x-icon name="bx-cog" class="{{ $iconClass }} {{ $iconMargin }}"
									x-bind:class="sidebarClose ? '[&]:mr-0' : ''" />
		...
		<div class="flex w-full justify-between gap-2 mt-auto">
			<div class="text-white" x-show="!sidebarClose" x-transition>
				...
				@else
					<x-icon name="bx-image-alt" class="w-[36px] h-[36px] cursor-help"
							x-tooltip.raw="{{ __('sidebar.avatar_help') }}" />
				@endif
			</div>

			@if(Auth::check())
				<div class="inline-flex justify-center flex-col items-center gap-1 text-xs" x-show="!sidebarClose"
					 x-transition>
					<span class="text-white">{{ $user->name }}</span>
					<span class="text-white">{{ $user->roles->first()->name }}</span>
				</div>
				<x-ui.button class="bg-primary-100" url="/logout">
					<x-icon name="bx-log-out" class="{{ $iconClass }}" />
				</x-ui.button>
			@endif
		</div>
	</div>
